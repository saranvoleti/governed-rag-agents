...
st.set_page_config(page_title="Governed RAG Agents", page_icon="🔍", layout="wide")
...
with col1:
    ...
    query = st.text_area("Your question", placeholder="What are the prior authorization requirements?", height=100)
    run = st.button("Analyze", disabled=not st.session_state.document_loaded, type="primary")
    ...
    if run and query:
        ...
        def on_trace(event, data):
            icons = {"agent_start": "🚀", "tool_call": "🔧", "tool_result": "📄",
                     "firewall_check": "🛡️", "firewall_rejected": "❌",
                     "firewall_result": "✅", "complete": "🏁"}
